...
<div class="fl-grid">
    ...
    <div>
        ...
        <div id="tb_content">
            ...
            function refreshPendingDownloads(){$.ajax({url:'ajax.php?ajax=pending_downloads',dataType:'json',timeout:5000,success:function(d){$('#pending-downloads-loading').hide();if(d.count>0){$('#pending-downloads-empty').hide();$('#pending-downloads-table').show();var h='';$.each(d.downloads,function(i,dl){var pc=dl.percent>=100?'var(--fl-success)':'var(--fl-accent)';h+='<tr class="flistmouseoff"><td title="'+dl.link+'"><b>'+dl.filename+'</b></td><td style="width:140px;"><div style="background:var(--fl-border);border-radius:4px;overflow:hidden;height:18px;"><div style="background:'+pc+';width:'+dl.percent+'%;height:100%;transition:width .3s;"></div></div><small>'+dl.percent+'%</small></td><td>'+dl.received+'</td><td>'+dl.total+'</td><td>'+formatElapsed(dl.elapsed)+'</td></tr>';});$('#pending-downloads-body').html(h);}else{$('#pending-downloads-table').hide();$('#pending-downloads-empty').show();}},error:function(){$('#pending-downloads-loading').hide();$('#pending-downloads-empty').text('Error loading downloads').show();}});}
